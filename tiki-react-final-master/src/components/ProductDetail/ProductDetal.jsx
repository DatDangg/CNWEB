...
function ProductDetail({product}) {
    ...
    useEffect(() => {
        // Set dữ liệu từ product được truyền vào hoặc từ dữ liệu trong file JSON
        ...
        else if (data && data.books && data.books.length > 0) { // Sử dụng dữ liệu từ file JSON
            setListImage([
                data.books[0].images[0].large_url,
                data.books[0].images[0].medium_url,
                data.books[0].images[0].thumbnail_url,
                data.books[0].images[0].base_url,
            ])
            setUnitPrice(data.books[0].original_price)
            setTotalCost(data.books[0].original_price)
        }
    }, [product])
                ...
                <div className="w-6/12 px-4">
                    <div className="product-detail__center">
                        <div className="center__heading">
                            ...
                            <span className="price">{product && product.original_price ? formatPrice(product.original_price) : '0'}<sup>đ</sup></span>
